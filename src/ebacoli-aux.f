c ebacoli-aux.f (supporting routines for eBACOLI-3, the source code
c contained in the file ebacoli.f.)
...
c Many of these routines have been modified for eBACOLI's purposes,
...
c user-defined subroutines describing eBACOLI's PDE system down
c to RES and JAC.
...
c However, the versions of these routines appearing in eBACOLI
...
c     followed by 71 "-"'s.
c
c     Further modifications were done to make the interface to eBACOLI
...
c     collocation and quadrature rules eBACOLI employs)
