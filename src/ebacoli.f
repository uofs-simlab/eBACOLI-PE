c This file contains the eBACOLI (Fortran77) source code (including the
...
c-----------------------------------------------------------------------
c Purpose:
c       The purpose of eBACOLI is to solve NPDE dimensional systems of
c       second order parabolic partial differential equations (PDEs)
c       in one space variable of the form:
c
c       dU            /                                        \
c       -- (t,x) = f | t, x, U(t,x), U (t,x), U   (t,x), V(t,x) | ,
c       dt            \               x         xx             /
c
c       dV            /                     \
c       -- (t,x) = f | t, x, U(t,x), V(t, x) | ,
c       dt            \                     /
c
...
c
c                /                                           \
c          b    | t, U(t,x_a), U (t,x_a), U(t,x_a), V (t,x_a) | = 0,
c           x_a  \              x                    x       /
c
c                /                                           \
c          b    | t, U(t,x_b), U (t,x_b), V(t,x_b), V (t,x_b) | = 0,
c           x_b  \              x                    x       /
c
...
c
c          dV
c          -- (t,x) = denotes the first partial derivative of V(t,x)
c          dt         with respect to the time variable t.
c
c          V (t,x) = denotes the first partial derivative of V(t,x)
c           x        with respect to space variable x.
c
c
c       Furthermore, the above system constitutes an NPDE dimensional
c       vector functions (that is dim(U) + dim(V)) while U alone
c       an NU dimensional vector function.
c
c
c       eBACOLI is a method of lines algorithm which uses bspline
...
c-----------------------------------------------------------------------
c Setup of eBACOLI:
c       eBACOLI requires that the user specifies the system of PDEs and
...
c
c       The calling sequence of eBACOLI is:
c
c       call ebacoli(t0, tout, atol, rtol, npde, nu,
c    &             kcol, nint, nintmx, x,
...
c       completion. Generally, the call to eBACOLI will be followed by a
...
c       eBACOLI are dealt with in detail below, but a quick summary is:
c
c       [t0, tout] is the time domain of the problem.
c       NPDE is the total number of components in the PDE system.
c       NU  is the number of components the parabolic system (dim(U)).
...
c       mflag(1:9) is used to control the operation of eBACOLI.
...
c
c       ( u represents the entire system stacked as [U V]' )
c
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c       NOTE: SHOULD INCLUDE AN NU IN THESE SIGNATURES AS WELL
c        - NOT NEEDED SO FAR AS OUR FUNCTIONS HAVEN'T USED LOOPS
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c
...
c
c                               After a successful return from eBACOLI,
...
c                               a new problem, and eBACOLI will have to
c                               be reinitialized.
c
...
c                               of eBACOLI with DASSL and which error
c                               estimation scheme eBACOLI will employ.
c
c                                         How to set mflag(1):
c                               On the initial call to eBACOLI with a
c                               new problem, set mflag(1) = 0, which
c                               indicates that eBACOLI and DASSL should
...
c                               return from eBACOLI, set mflag(1) = 1,
...
c                               If the user wishes, eBACOLI will return
...
c                               If the user wants to run eBACOLI with the
...
c                               If the user wants to eBACOLI use finite
...
c                               the operation of DASSL within eBACOLI.
c
c-----------------------------------------------------------------------
...
c                               steps since we restart eBACOLI in the
...
c                               asks eBACOLI to return the computed
c                               solution.
...
c                               times after eBACOLI starts the initial
c                               step.
c
        integer                 istalr
        parameter              (istalr = 12)
...
c                               eBACOLI has taken before the latest cold
c                               start.
c
        integer                 icolds
        parameter              (icolds = 14)
c                               ipar(icolds) is the number of the times
c                               when eBACOLI performs a cold start after
c                               remeshing.
c
        integer                 imxord
        parameter              (imxord = 15)
...
c
        integer                 iresto
        parameter              (iresto    = 57)
c                               rpar(ipar(iresto)) holds the temporary storage
c                               for copying top and bottom PDE blocks before
c                               reinit.
c
c-----------------------------------------------------------------------
...
c     caljac. This value can in theory change between calls to eBACOLI.
      ipar(imflg9) = mflag(9)
...
c     outputs y to rpar(ipar(irwork)+40).
     ...
     &     rpar(ipar(ibasi)), rpar(ipar(irwork)+40), rpar(ipar(iabtop)),
...
c-----------------------------------------------------------------------
  600 continue
      write(6,9999) 'ERROR: eBACOLI runtime error in time stepping.'
      write(6,9999) '       An error code and message should have'
      write(6,9999) '       been issued by DASSL.'
      return
  610 continue
      if (istart .eq. 1) then
         write(6,9998) 'ERROR: eBACOLI has remeshed ', maxrsh
...
c-----------------------------------------------------------------------
 ...
 735  continue
  ...
  790 continue
      write(6,9999) 'ERROR: eBACOLI input violation.'
      write(6,9999) 'IDID .ne. 1, on a continuation call of eBACOLI'
...
c       the nderiv-th derivative) at xsol and at eBACOLI's output time.
c
c       Work Storage:
...
c
c       TESTING - to explore results if ODE systems do not contribute to
c                 the error. Generally don't want to expose this in API.
c                 = 1  . Zeros out ODE contributions to error
c                 = 0  . Does not zero out ODE contributions to error
        integer                 ienoode
        parameter              (ienoode = 0)
c-----------------------------------------------------------------------
c Subroutine Parameters:
...
c     issue. (This is also the case in the original eBACOLI.)
      if (EST .eq. 1) then
         do j = 1, npde
c           i = 1
            rL = zero
...
c     Stop now if nint > nintmx and let ebacoli pick up the error
...
c
        ...
        external                bndxb
        external                difbxb
c                               refer to the preamble of eBACOLI.
c
c       Work Storage:
        integer                 ipivot(neq)
...
c                               The TOP block of the matrix A.
c
...
c                               The BOT block of the matrix A.
...
c     value at the left end point
c     -> setting diagonal elements in abdtop
      do 40 i = 1, npde
         ii = (i-1) * npde + i
         abdtop(ii) = fbasis(1,1,1)
   40 continue
...
c
     ...
     &            fbasis(1,1,ncpts),3)
      ...
      do 90 i = 1, npde
         ii = ((i-1)+npde)*npde + i
         abdbot(ii) = fbasis(kcol+nconti,1,ncpts)
   90 continue


c-----------------------------------------------------------------------
c     Copy the TOP block of the collocation matrix into temporary storage.
...
c     Copy the BOT block of the collocation matrix into temporary storage.
      call dcopy(npde*npde*nconti,abdbot,1,work(iabdbt),1)

c     Check whether both boundary conditions are Dirichlet boundary
...
c     -> residual
      do i = 1,nv
         ...
         do  j = 1, npde*nconti
            jj = mm + (j-1)*npde
            work(ii) = work(ii) + abdtop(jj)*y(j)
         enddo
      enddo

c     Evaluate u and derivatives, store in
c       work(iu), work(iux), work(iuxx)
      call eval(npde,kcol,kcol+2,1,ncpts,work(iu),work(iux),
     &          work(iuxx),fbasis(1,1,1),y)
c     Evaluate left BC, stores in work(idelta)
...
c     Update the values at the right boundary for the ODEs
      do i = 1, nv
         ...
         do  j = 1, npde*nconti
            jj = mm + (j-1)*npde
            work(ii) = work(ii) + abdbot(jj)*y(neq-(npde*nconti)+j)
         enddo
      enddo

c     Update the values at the right boundary, store in
c       work(iu), work(iux), work(iuxx)
      call eval(npde,kcol,ncpts,ncpts,ncpts,work(iu),work(iux),
     &          work(iuxx),fbasis(1,1,ncpts),y)
c     Evaluate right BC, stores in work(idelta)
...
c     Replace the value of yprime for the PDE, this requires call to difbxa AGAIN!
         call eval(npde,kcol,kcol+2,1,ncpts,work(iu),work(iux),
     &        work(iuxx),fbasis(1,1,1),y)
         call difbxa(t0, work(iu), work(iux), work(idbdu),
     &        work(idbdux), work(idbdt), npde)
...
c     conditions of the PDEs, into yprime.
c     NOTE:
c       It is important that this is after the above calls because they write to the whole system.
c       Now the PDE part is overwritten with its correct values
      do 100 i = 1, nu
...
c     Generate the value of yprime for the ODEs - Right BC
     ...
     &        work(iuxx),fbasis(1,1,jj),y)
         call f(t0,xcol(jj),work(iu),work(iux),work(iuxx),yprime(mm),
     &        npde)
c     Replace the value of yprime for the PDE, this requires call to difbxb AGAIN!
         call eval(npde,kcol,ncpts,ncpts,ncpts,work(iu),work(iux),
     &        work(iuxx),fbasis(1,1,ncpts),y)
         call difbxb(t0,work(iu),work(iux),work(idbdu),
     &        work(idbdux),work(idbdt),npde)
...
c     Solve the linear system. This gives yprime(t0)
  ...
  999 continue
     ...
     &                  fbasis, y, abdtop, abdblk, abdbot, icflag)
...
c-----------------------------------------------------------------------
c     Copy the middle of the collocation matrix into temporary storage.
      call dcopy(npde*npde*nconti,abdtop(1),1,work(iabdtp),1)
      call dcopy(nels*nint,abdblk,1,work(iabdbk),1)
      call dcopy(npde*npde*nconti,abdbot(1),1,work(iabdbt),1)
...
c                               rpar is the eBACOLI floating point work
c                               array.
c
        integer                 ipar(*)
c                               rpar is the eBACOLI integer work array.
c
        external                derivf
        external                bndxa
        external                difbxa
...
c     Scale entire delta by negative 1
      call dscal(neq, negone, delta(1), 1)
...
c     ODE for the left boundary: delta(nu+1:npde)
      do i = 1, nv
         ...
         do 61 j = 1, nconti*npde
            jj = (j-1)*npde + ii
            delta(ii) = delta(ii) + abdtop(jj) * yprime(j)
 61      continue
      enddo
...
c Constants
      integer                 mxkcol
      parameter              (mxkcol = 10)
c     ebacoli can have up to 10 collocation points per subinterval.
c
      integer                 nconti
      parameter              (nconti = 2)
...
c     the main eBACOLI loop. So while there is unnecessary work done here
...
c        conditions used in eBACOLI models, b(t,u,ux) = 0
